...
//PMI統計圖表
	...
	//貨幣供給M1B
	function moneySupplyM1B(data){

		var className=".moneySupply";
		$(className).html("");
		var width = 500 ; // 寬
		var height = 250 ; // 高
		...
		x.domain([data[0].date, data[data.length - 1].date]);
		y.domain(d3.extent(data, function(d) { return d.M1B; }));
	...
	//貨幣供給M2
	function moneySupplyM2(data){

		var className=".moneySupply2";
		$(className).html("");
		console.log('1');
		var width = 500 ; // 寬
		var height = 250 ; // 高
		...
		x.domain([data[0].date, data[data.length - 1].date]);
		y.domain(d3.extent(data, function(d) { return d.M2; }));
		...
		//tips
		var tips = svg.append('g').attr('class', 'tips3');
		...
		var g = svg.selectAll('circle')
		  ...
		  .on("mouseover", function(d){
			  ...
			  var cy = m[1] - margin.top + 20;
			 textBox.text(d.M2);
			 d3.select('.tips3').attr('transform', 'translate(' + cx + ',' + cy + ')');
			 d3.select('.tips3').style('display', 'block');
			 d3.select(this).transition().duration(500).attr('r', 5);
		  })
		  .on('mouseout', function() {
		    d3.select('.tips3').style('display', 'none');
		    d3.select(this).transition().duration(500).attr('r', 3.5);
		  });
